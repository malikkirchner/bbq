 ...
 ****************************************************************************************/
template <typename BT, std::size_t N> class SUFactory : public MatrixFactory<SU<BT, N>> {
protected:
    std::mt19937 uni;
...
public:
    ...
    virtual matrix_type generateRandom() noexcept final {
        ...
        if (N == 1) {
            ...
            res(0, 0).imag(std::sqrt(1.0 - cos_alpha * cos_alpha));
        } else {
            ...
            for (unsigned i = 0; i < N - 1; i++) {
                for (unsigned j = i + 1; j < N; j++) {
                    ...
                    cos_alpha =
                        (BT)1.0 -
                        (BT)(1.0 + (2.0 * (BT)(rand() % 2) - 1.0) * std::cos(alpha)) * rangemod;
                    sin_alpha = (BT)std::sqrt(1.0 - cos_alpha * cos_alpha);
                    sin_theta = (BT)std::sqrt(1.0 - cos_theta * cos_theta);
                    cos_phi   = (BT)std::cos(phi);
                    sin_phi   = (BT)std::sin(phi);

                    coeff.c[0] = sin_alpha * sin_theta * cos_phi;
                    coeff.c[1] = sin_alpha * sin_theta * sin_phi;
                    coeff.c[2] = sin_alpha * cos_theta;
